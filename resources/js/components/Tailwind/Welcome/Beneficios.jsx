          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 items-center gap-x-16 gap-y-8 xl:gap-20 4xl:gap-24 px-8" aria-hidden="true">
             {brands.map((logo) => (
                  <div className='flex flex-col items-center gap-2 max-w-40 text-center h-36'>
                      <div className='w-20 h-20 4xl:w-24 4xl:h-24 rounded-full bg-[#ff9003] flex flex-col justify-center items-center'>
                        <img 
                          key={`${logo.id}-clone`}
                      ...
                      <h2 className='font-sora text-[#5e5e60] text-sm 2xl:text-base 4xl:text-xl tracking-tight font-medium'>{logo.name}</h2>
                  </div>
            ))}
          </div>
