            ...
            <div className="relative w-full px-[5%] 4xl:px-[8%] gap-10 xl:gap-16 flex flex-col items-center py-10 xl:py-20">
                ...
                {!!firstSection?.visible && (
                    <div className="w-full flex flex-col items-center justify-center gap-5 max-w-3xl 4xl:max-w-4xl text-center">
                        <h1 className="font-sora text-[#324050] text-3xl sm:text-4xl 2xl:text-5xl 4xl:text-6xl font-semibold tracking-tight !leading-tight">
                            {firstSection?.name}
                        </h1>
                        {firstSection?.subtitle && (
                            <p className="font-dmsans text-[#707c84] text-base sm:text-lg 2xl:text-xl 4xl:text-2xl tracking-normal font-light">
                                {firstSection?.subtitle}
                            </p>
                        )}
                    </div>
                ...
                <div className="flex flex-col xl:flex-row gap-10 w-full items-center justify-center">
                    <div className="w-full xl:w-1/2 flex flex-col gap-2 justify-center items-start">
                        {!!secondSection?.visible && (   
                            <>
                                <h2 className="font-sora text-[#324050] text-3xl sm:text-4xl 2xl:text-4xl 4xl:text-5xl font-semibold tracking-tight !leading-tight">
                                    {secondSection?.name}
                                </h2>
                        ...
                        <div className="flex flex-col sm:flex-row justify-start items-start gap-5 md:gap-8 lg:gap-5 2xl:gap-12 4xl:gap-14 w-full mt-4">
                            ...
                            {!!thirdSection?.visible && (
                                <div onMouseEnter={() => setActiveImage(1)}
                                    ...
                                    ${activeImage === 1 ? 'border-[#5e5e60]' : ' border-[#5e5e60] border-opacity-20'}`}
                                >
                                    <div className="w-14 h-14 4xl:w-16 4xl:h-16 rounded-full bg-[#7c231c] flex flex-col justify-center items-center">
                                        <img 
                                            className="w-8 h-8 4xl:w-10 4xl:h-10" 
                                            src={`/api/aboutus/media/${thirdSection?.icon}`} 
                                            alt="" 
                                            aria-hidden="true" 
                                        />
                                    </div>
                                    <h3 className="font-sora text-[#324050] text-xl 2xl:text-2xl 4xl:text-3xl font-semibold tracking-tight !leading-tight">
                                        {thirdSection?.name}
                                    </h3>
                                    <HtmlContent
                                        className="prose font-dmsans text-[#324050] text-base 2xl:text-lg 4xl:text-xl tracking-normal font-light"
                            ...
                            {!!fourSection?.visible && (
                                <div onMouseEnter={() => setActiveImage(2)}
                                    className={`w-full lg:w-1/2 flex flex-col justify-center items-start border-2 rounded-xl p-6 gap-4 transition-all duration-300 cursor-default
                                    ${activeImage === 2 ? 'border-[#5e5e60]' : ' border-[#5e5e60] border-opacity-20'}`}
                                >
                                    <div className="w-14 h-14 4xl:w-16 4xl:h-16 rounded-full bg-[#7c231c] flex flex-col justify-center items-center">
                                        <img 
                                            className="w-9 h-9 4xl:w-12 4xl:h-12" 
                                            src={`/api/aboutus/media/${fourSection?.icon}`} 
                                            alt="" 
                                            aria-hidden="true" 
                                        />
                                    </div>
